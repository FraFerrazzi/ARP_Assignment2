...
{
    char *ip = "127.0.0.1";
    int sockfd, newsockfd, portno, clilen, pidServer;
    ...
    //getting PidServer to send to ClientSocket
    pidServer = getpid();
    char buf_pidServer[20];
    sprintf(buf_pidServer, "%d", pidServer);
    //asking the user to chose the port number
    printf("\nChose a value for the port number\n");
    fflush(stdout);
    scanf("%d", &portno);
    char buf_portno[20];
    sprintf(buf_portno, "%d", portno);
    // arglist
    char * arg_list[]={"./ClientSocket", buf_pidServer, buf_portno, (char*)NULL};
