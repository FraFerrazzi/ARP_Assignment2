...
int main (int argc, char *argv[]) 
{
	...
	if (pid > 0) //I'm in the parent process
	{
		...
		for (int i=0; i < size; i++)
		{
			...
			//printf("scrivo il mega numero [%d]\n", i);
			//fflush(stdout);
		}
		printf("Culo1");
		fflush(stdout);
	...
	if(pid == 0)//I'm in the child process
	{
		...
		for (int i = 0; i < size; i++){
			...
			//printf("sto leggendo il mega[%d]", i);
			//fflush(stdout);
		}
		printf("Culo2");
		fflush(stdout);
		...
		close(fd_unnamed[ReadEnd]);
		//time in nanosec
		float time_taken = ((t_end.tv_sec*1000000 + t_end.tv_usec) - (t_start.tv_sec*1000000 + t_start.tv_usec));
