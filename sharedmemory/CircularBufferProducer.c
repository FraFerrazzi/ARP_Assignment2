...
int main(int argc, char *argv[]) 
{
    ...
    for (int i=0; i < size; i++)
	{
		...
		while (j < MUL_SIZE)
		{
            ...
            while (k < CBUFF_SIZE)
            {
                circular_buffer[k] = data_array[j];
                //wait until array is full
                sem_wait(sem_id2);
                // wait cli
                sem_wait(sem_id1);
                memcpy(addr, &circular_buffer[k], sizeof(circular_buffer[k]));
                // start cli
                sem_post(sem_id1);
                //increment until array is full
                sem_post(sem_id3);
                printf("cb:%d  ", circular_buffer[k]);
                printf("da:%d  ", data_array[j]);
                fflush(stdout);
                addr += sizeof(circular_buffer[k]);
                j ++;
                k ++;
            }
		}
